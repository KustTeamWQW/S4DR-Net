...
from model import S4DRNetv2_v1
import numpy as np
...
def train(args, io):
    train_loader = DataLoader(
        S3DIS(partition='train', num_points=args.num_points, test_area=args.test_area, split_num=args.split_num),
        num_workers=8, batch_size=args.batch_size, shuffle=True, drop_last=True)

    test_loader = DataLoader(
        S3DIS(partition='test', num_points=args.num_points, test_area=args.test_area, split_num=args.split_num),
        num_workers=8, batch_size=args.test_batch_size, shuffle=False, drop_last=False)

    device = torch.device("cuda" if args.cuda else "cpu")

    model = S4DRNetv2_v1(args).to(device)
    print(str(model))
    ...
    if not args.pretrain:  # 第二步
        pretrained_dict = torch.load(f'/best_pretrain_model.t7')  ##
    ...
    for epoch in range(args.epochs):
        ...
        for data, seg, p2v_indices, part_distance, part_distance_sp, part_rand_idx in tqdm(train_loader):
            ...
            loss_seg = cal_loss(seg_pred.view(-1, 9), seg.view(-1, 1).squeeze(), smoothing=True)
        ...
        with torch.no_grad():
            for data, seg, p2v_indices, part_distance, part_distance_sp, part_rand_idx in tqdm(test_loader):
                ...
                # Calculate segmentation loss
                loss_seg = cal_loss(seg_pred.view(-1, 9), seg.view(-1, 1).squeeze())
