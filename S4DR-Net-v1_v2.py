...
from model import S4DRNetv1_v2
import numpy as np
...
def calculate_metrics(true_labels, pred_labels, num_classes=8): #
    class_acc = []
    class_recall = []
    class_f1 = []
...
def train(args, io):
    ...
    model = S4DRNetv1_v2(args).to(device)
    print(str(model))
    ...
    for epoch in range(args.epochs):
        ...
        for data, seg, p2v_indices, part_distance, part_distance_sp, part_rand_idx in tqdm(train_loader):
            ...
            loss = (loss_seg / loss_seg.detach()) + (hop_loss / hop_loss.detach()) + (
                    hop_loss_sp / hop_loss_sp.detach()) 
            # print(loss_seg)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--gpu', type=str, default="6", metavar='N',
                        help='Cuda id')
    # ---------------------------------------------------------
